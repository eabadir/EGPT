...
let DEBUG_PROMOTION = !!(typeof process !== 'undefined' && process.env && process.env.EGPT_TRACE_C1);

/**
 * Mulberry32 — seedable 32-bit PRNG. Returns a closure that produces [0, 1) floats.
 * Identical output can be reproduced in Rust with u32 wrapping arithmetic.
...
function mulberry32(seed) {
    ...
    fn_rng.getState = function() { return state; };
    fn_rng.getCallCount = function() { return callCount; };
    fn_rng.resetCallCount = function() { callCount = 0; };
    return fn_rng;
}

/**
 * Module-level RNG function. Defaults to Math.random.
 * When a seeded EGPTUniverse is created, this is replaced with a Mulberry32 instance
 * so that all Frame methods (which call _rng()) produce deterministic output.
 */
...
class EGPTUniverse {
     ...
     * @param {number|null} seed - PRNG seed for deterministic runs. null = use Math.random() (default null)
     */
    constructor(fps = 60, iframe_interval_seconds = 1, withInterQuantumCollisions = false, withBonding = false, withFrames = true, emergentPhysics = 0, minMBFillRate = .1, universe_rect = null, wavelength_constant = WAVELENGTH_CONSTANT, lowest_dimension = 0, seed = null) {
        ...
        /* Seedable PRNG — when seed is provided, all randomness is deterministic.
         * The module-level _rng is replaced so Frame methods automatically use it. */
    ...
    doTick() {
        ...
        for (let dim of allDimensions) {
            ...
            if (this.withInterQuantumCollisions && dim.layer === this.lowest_dimension) {
                ...
                let rejected = localQTree.insertAll(dim.frames);
                if (typeof process !== 'undefined' && process.env && process.env.EGPT_TRACE_C1) {
                    let rejectedIds = rejected.map(f => f.quantum_id).sort((a,b) => a-b);
                    let storedIds = dim.frames.map(f => f.quantum_id).filter(id => !rejectedIds.includes(id)).sort((a,b) => a-b);
                    console.log(`  QuadTree: ${dim.frames.length} frames, ${rejected.length} rejected, ${storedIds.length} stored`);
                    console.log(`  Stored IDs: [${storedIds.join(',')}]`);
                    console.log(`  Rejected IDs: [${rejectedIds.join(',')}]`);
                }
                if (typeof process !== 'undefined' && process.env && process.env.EGPT_TRACE_C1) {
                    console.log(`  RNG after QuadTree insert: state=${this._prng.getState()} calls=${this._prng.getCallCount()}`);
                }
                for (let p of dim.frames) {
                    let range = new Circle(p.rect.x, p.rect.y, p.r);
                    let neighbors = localQTree.query(range);
                    if (typeof process !== 'undefined' && process.env && process.env.EGPT_TRACE_C1 && neighbors.length > 1) {
                        console.log(`  query id=${p.quantum_id} (${p.rect.x.toFixed(1)},${p.rect.y.toFixed(1)}) -> neighbors=[${neighbors.map(n => n.quantum_id).join(',')}]`);
                    }
                    p.checkCollision(neighbors, true, this.withBonding, this.tick);
                }
                if (typeof process !== 'undefined' && process.env && process.env.EGPT_TRACE_C1) {
                    console.log(`  RNG after StepC1: state=${this._prng.getState()} calls=${this._prng.getCallCount()}`);
                }
            }
            ...
            if (typeof process !== 'undefined' && process.env && process.env.EGPT_TRACE_C1) {
                console.log(`  RNG after dim.update(layer=${dim.layer}): state=${this._prng.getState()} calls=${this._prng.getCallCount()}`);
            }
...
class Frame {
    ...
    constructor(layer, rect, vx = 0, vy = 0, wrap = true, boundary_rect = null, parent = null, emergentPhysics = false) {
        ...
        this.charge = _rng() < 0.5 ? Charge.POSITIVE : Charge.NEGATIVE;
    ...
    static makeLeafFrame(left, top, level, wavelength, vx = 0, vy = 0, wrap = true, charge = null, boundary_rect = null, color_override = null, emergentPhysics = false) {
        ...
        frame.charge = charge !== undefined && charge !== null ? charge : (_rng() < 0.5 ? Charge.POSITIVE : Charge.NEGATIVE);
    ...
    reset(x, y, vx, vy) {
        ...
        this.collided = false;
        this.is_alive = true;
        //this.charge = (this.original_charge !== undefined && this.original_charge !== null) ? this.original_charge : (_rng() < 0.5 ? Charge.POSITIVE : Charge.NEGATIVE);
        this.charge = (_rng() < 0.5 ? Charge.POSITIVE : Charge.NEGATIVE);
    ...
    move(t, boundary_rect) {
        ...
        if (!this.is_stationary) {
            ...
            if (!this.emergentPhysics && !BROWNIAN_MOTION && !this.is_stationary) {
                ...
                let _rng_val = _rng();
                if (typeof process !== 'undefined' && process.env && process.env.EGPT_TRACE_C1 && this.quantum_id < 3) {
                    console.log(`    MOVE id=${this.quantum_id} vx=${this.vx.toFixed(1)} vy=${this.vy.toFixed(1)} charge=${this.charge} local_time=${this.local_time} wl=${this.wavelength} p_up=${this.p_up.toFixed(4)} rng=${_rng_val.toFixed(6)} spin=${this.spin}`);
                }
                let is_heads = _rng_val < this.p_up ? 1 : -1;
                ...
                let brownian_x = Math.floor(_rng() * 3) - 1;
                let brownian_y = Math.floor(_rng() * 3) - 1;
                ...
                this.dx = (_rng() < prob_vx ? normalized_vx : 0) + brownian_x;
                this.dy = (_rng() < prob_vy ? normalized_vy : 0) + brownian_y;
                this.vx += this.dx;
                this.vy += this.dy;
            ...
            if (this.parent) {
                ...
                // Free brownian particles: successful displacement expends momentum
                // on both axes, keeping vx/vy at their pre-move values. Momentum is
                // a persistent drift tendency (voltage analog); moves spend it, wall
                // collisions (WALL type) reposition without touching it.
                this.vx -= this.dx;
                this.vy -= this.dy;
            }
        }
    ...
    bounceBrownian(other) {
        ...
        this.vx = _rng() * (_rng() > 0.5 ? 1 : -1);
        this.vy = _rng() * (_rng() > 0.5 ? 1 : -1);
    ...
    bounce(other) {
        ...
        let quantumCenterX = this.rect.x + this.rect.w / 2;
        let quantumCenterY = this.rect.y + this.rect.h / 2;
        ...
        let combinedHalfWidths = this.rect.w / 2 + other.rect.w / 2;
        let combinedHalfHeights = this.rect.h / 2 + other.rect.h / 2;
        this.local_time = 0;
        // Check for collision
    ...
    checkCollision(others, withbouncing = false, withbonding = false) {
        // Sort by quantum_id for deterministic collision order (cross-engine parity)
        others.sort((a, b) => a.quantum_id - b.quantum_id);
        ...
        for (let other of others) {
            ...
            if (d < other.r / 2 + this.r / 2) {
                if (typeof process !== 'undefined' && process.env && process.env.EGPT_TRACE_C1) {
                    console.log(`  StepC1: id=${this.quantum_id} (${this.rect.x.toFixed(1)},${this.rect.y.toFixed(1)}) <-> id=${other.quantum_id} (${other.rect.x.toFixed(1)},${other.rect.y.toFixed(1)}) d=${d.toFixed(4)}`);
                }
...
class Dimension {
    ...
    createParentFrame(childA, childB) {
        ...
        if (DEBUG_PROMOTION) {
            console.log(`[PROMOTE] createParentFrame dim=${this.layer} childA=${childA.quantum_id}(${childA.rect.x.toFixed(1)},${childA.rect.y.toFixed(1)}) childB=${childB.quantum_id}(${childB.rect.x.toFixed(1)},${childB.rect.y.toFixed(1)}) span=${spanW.toFixed(1)}x${spanH.toFixed(1)} limit=${this.mb_w}x${this.mb_h}`);
        }
        if (spanW > this.mb_w || spanH > this.mb_h) {
            if (DEBUG_PROMOTION) {
                console.log(`[PROMOTE] createParentFrame FAIL: span exceeds limit`);
            }
            return null;
        }
    ...
    handleStructurePromotion(frameA, frameB) {
        ...
        if (DEBUG_PROMOTION) {
            console.log(`[PROMOTE] handleStructurePromotion dim=${this.layer} A.id=${frameA.quantum_id} B.id=${frameB.quantum_id} A.alive=${frameA.is_alive} B.alive=${frameB.is_alive} A.parent=${!!frameA.parent} B.parent=${!!frameB.parent}`);
        }
        if (!frameA.is_alive || !frameB.is_alive) {
            if (DEBUG_PROMOTION) console.log(`[PROMOTE] REJECT: not alive`);
            return;
        }

        if (frameA.parent || frameB.parent) {
            if (DEBUG_PROMOTION) console.log(`[PROMOTE] REJECT: has parent`);
            return;
        }
        ...
        if (nextLayer === undefined) {
            if (this.universe.emergentPhysics) {
                ...
                if (DEBUG_PROMOTION) console.log(`[PROMOTE] REJECT: no nextLayer, emergentPhysics=false`);
                return;
            }
        }
        ...
        if (!parentDim) {
            if (DEBUG_PROMOTION) console.log(`[PROMOTE] REJECT: parentDim missing for nextLayer=${nextLayer}`);
            return;
        }

        let created = parentDim.createParentFrame(frameA, frameB);
        if (DEBUG_PROMOTION && created) console.log(`[PROMOTE] SUCCESS: created parent in dim=${nextLayer} id=${created.quantum_id}`);
        if (DEBUG_PROMOTION && !created) console.log(`[PROMOTE] REJECT: createParentFrame returned null`);
    }
    ...
    update(t, large_object_list = null) {
        ...
        if (typeof process !== 'undefined' && process.env && process.env.EGPT_TRACE_C1) {
            // Log .other BEFORE insertAll to detect leaking from Step C.1
            let preOthers = this.frames.filter(f => f.other).map(f => `${f.quantum_id}->${f.other.quantum_id}`);
            if (preOthers.length > 0) console.log(`  dim${this.layer}.update PRE-insertAll .other: [${preOthers.join(', ')}]`);
        }
        let collisionList = qtree.insertAll(this.frames);
        if (typeof process !== 'undefined' && process.env && process.env.EGPT_TRACE_C1) {
            for (let f of this.frames) {
                console.log(`    pre-insert id=${f.quantum_id} pos=(${f.rect.x.toFixed(1)},${f.rect.y.toFixed(1)})`);
            }
            console.log(`  dim${this.layer}.update frames_order=[${this.frames.map(f=>f.quantum_id).join(',')}]`);
            console.log(`  dim${this.layer}.update collisionList=${collisionList.length} withOther=${collisionList.filter(f=>f.other).length}`);
            for (let f of collisionList) {
                console.log(`    insert id=${f.quantum_id}: Err(existing=${f.other ? f.other.quantum_id : 'none'})`);
            }
            // Log .other AFTER insertAll
            let postOthers = this.frames.filter(f => f.other).map(f => `${f.quantum_id}->${f.other.quantum_id}`);
            console.log(`  dim${this.layer}.update POST-insertAll .other: [${postOthers.join(', ')}]`);
        }
        ...
        if (this.universe.withInterQuantumCollisions && collisionList.length > 0) {
            // Sort by quantum_id for deterministic collision order (cross-engine parity)
            collisionList.sort((a, b) => a.quantum_id - b.quantum_id);
            if (DEBUG_PROMOTION && this.universe.tick % 60 === 0) {
            ...
            for (let frame of collisionList) {
                if (frame.other) {
                    ...
                    if (typeof process !== 'undefined' && process.env && process.env.EGPT_TRACE_C1) {
                        console.log(`    collision id=${frame.quantum_id}<->id=${frame.other.quantum_id} overlaps=${overlaps} canPromote=${canPromote}`);
                    }
...
if (typeof module !== 'undefined') {
    module.exports = { EGPTUniverse, Frame, Dimension, TickData, FrameData, DimensionData, UniverseData, LightColor, Charge, mulberry32 };
