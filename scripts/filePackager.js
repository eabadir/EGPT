// Base directory is the repo root (one level up from scripts/)
...
const readFileContent = (filePathObj) => {
  ...
  const absolutePath = path.resolve(REPO_ROOT, filePathObj.path);
...
const interactiveFileSelection = async () => {
  ...
  return new Promise(async (resolve) => {
    rl.question('Enter the base folder path (relative to the script): ', async (baseFolder) => {
      ...
      const fullPath = path.resolve(REPO_ROOT, basePath);
      ...
      rl.question('Enter the manifest file name to save (default: new_manifest.json): ', (manifestFileName) => {
        ...
        const manifestPath = path.join(REPO_ROOT, 'temp', fileName);
...
const createOutputFilesWithPaths = (pathsConfig, outputDir = '', inputManifestPath = '') => {
  ...
  pathsConfig.forEach(item => {
    if (!item.includeFile) {
      ...
      excludedPaths.set(path.resolve(REPO_ROOT, item.path), item.outputFile);
    }
  });
  ...
  pathsConfig.forEach(item => {
    if (item.includeFile) {
      const absolutePath = path.resolve(REPO_ROOT, item.path);
      try {
        const stat = fs.statSync(absolutePath);
        if (stat.isDirectory()) {
  ...
  let baseDir = path.resolve(REPO_ROOT);
