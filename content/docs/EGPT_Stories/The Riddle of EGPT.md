...
Any problem in the class NP can be expressed as a CNF satisfiability problem — a list of constraints over Boolean variables. This is the essence of the Cook-Levin theorem, which EGPT formalizes in its own framework.

*   **Lean Citation:** `EGPT/Complexity/PPNP.lean`
    *   `theorem EGPT_CookLevin_Theorem : IsNPComplete L_SAT_Canonical` proves that SAT is the canonical NP-Complete problem in the EGPT framework.
