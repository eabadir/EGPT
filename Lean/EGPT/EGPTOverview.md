...
* **Lean Citation:** The definition of `P` in `EGPT/Complexity/PPNP.lean`. The constructive function `walkCNFPaths` in `EGPT/Complexity/TableauFromCNF.lean` demonstrates that such certificates can always be built deterministically via the `PathToConstraint` insight.
