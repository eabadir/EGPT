...
* **4.2. The NP Class:** A language is in `NP` if a "yes" instance is certified by the **mere existence** of an externally provided, polynomially-bounded `SatisfyingTableau`.
...
* **4.3. The P Class:** A language is in `P` if a "yes" instance is certified by the **deterministic construction from CNF alone** of a polynomially-bounded `SatisfyingTableau`. The witness is built by `constructTableauFromCNF`, not provided externally.
...
* **A Non-Trivial Proof:** P and NP are structurally distinct definitions (deterministic construction from CNF alone vs. externally provided certificate). The proof shows their equivalence by demonstrating that `constructTableauFromCNF` can always build the witness without external input.
* **Lean Citation:** `theorem P_eq_NP : P = NP` in `EGPT/Complexity/PPNP.lean`.
* **Explanation:** The hard work was done in the preceding theorems (`L_SAT_in_NP` and `L_SAT_in_P`), which proved that the structurally distinct definitions yield the same set of languages. P requires deterministic construction from CNF alone; NP requires an externally provided certificate. The proof shows that if a polynomially-bounded certificate *can exist* (NP), `constructTableauFromCNF` *can always build it* from the CNF alone in polynomial time (P). The witness is separated from the construction -- non-circularity is guaranteed because the construction algorithm does not receive the NP certificate as input.
