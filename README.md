...
In EGPT's information space, a CNF formula is a list of addresses, and each address *is* the path to reach that variable. The certificate for any satisfiable CNF has complexity bounded by n². `P` and `NP` are syntactically identical definitions — the proof completes with `Iff.rfl`.

The information-theoretic argument: RET proves information is conserved. You cannot construct a problem whose solution requires more information than the problem statement contains. The distinction between "search" and "verification" vanishes.
...
6. **P = NP** — [`Complexity/PPNP.lean`](Lean/EGPT/Complexity/PPNP.lean): `P` and `NP` are syntactically identical. `P_eq_NP` completes with `Iff.rfl`.
