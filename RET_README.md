...
1. **Maxwell–Boltzmann (MB):**  
   ...
   • $\Omega_{MB}$ has size $N^M$ (each of the $M$ labels chooses one of $N$ boxes).  
...
2. **Fermi–Dirac (FD):**  
   ...
   • $\Omega_{FD}$ has size $\binom{N}{M}$ (choose which $M$ of the $N$ boxes are occupied).  
...
3. **Bose–Einstein (BE):**  
   ...
   • $\Omega_{BE}$ has size $\binom{N+M-1}{M}$ (standard “stars and bars” count).  
